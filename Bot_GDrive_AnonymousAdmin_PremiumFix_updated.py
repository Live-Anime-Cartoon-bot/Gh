...
def _six_job_count(user_id):
    return len(_six_user_active.get(int(user_id), set()))

def _six_global_count():
...
def _six_acquire(user_id, job_type):
    ...
    uid = int(user_id)
    user_count = _six_job_count(uid)

    if user_count >= SIX_JOB_MAX_PER_USER:
        return None, (
            "⛔ **Maximum 5 active jobs allowed per user.**\n"
            f"Your active jobs: `{user_count}/5`"
        )

    global_count = _six_global_count()

    if global_count >= SIX_JOB_MAX_GLOBAL:
        return None, (
            "⏳ **Server busy.**\n"
            f"Global active jobs: `{global_count}/6`\n"
            f"Your active jobs: `{user_count}/5`\n"
...
def is_anonymous_admin_enabled() -> bool:
    ...
    data = _load_json(ANON_ADMIN_FILE, {"enabled": False})
...
async def cookies_document_handler(client: Client, message: Message):
    ...
    status = await _reply_auto_delete(message, AUTO_DELETE_5MIN, "⬇️ Downloading cookie file...")
    try:
...
def _wm_pos_keyboard(uid: int, selected: str) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup([
        ...
        [
            InlineKeyboardButton(
                "▶️ Start",
                callback_data=f"wmp:{uid}:start"
            ),
            InlineKeyboardButton(
                "❌ Cancel",
                callback_data=f"wmp:{uid}:cancel"
            ),
        ],
    ])
...
async def watermark_cmd(client: Client, message: Message):
    ...
    await message.reply_text(
        ...
        reply_markup=_wm_pos_keyboard(user_id, "normal1"),
    )
...
@app.on_callback_query(filters.regex(r"^wms:(\d+):(wm1|wm2|normal1|normal2|center|delete)$"))
async def wms_callback(client: Client, cq: CallbackQuery):
    ...
    try:
        await cq.message.edit_text(
            ...
            f"Style: `{action}`\n\n"
            f"Choose position:",
            reply_markup=_wm_pos_keyboard(uid, session.get("pos", "bottom_right")),
        )
    except Exception:
        pass


@app.on_callback_query(filters.regex(r"^wmp:(\d+):(normal1|normal2|center|start|cancel)$"))
...
async def premium_add_cmd(_client: Client, message: Message):
    anonymous_admin = _is_official_anonymous_admin(message)
    uid = OFFICIAL_ANONYMOUS_GROUP_ID if anonymous_admin else (message.from_user.id if message.from_user else 0)
